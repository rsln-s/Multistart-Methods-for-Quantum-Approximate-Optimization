...
def run_angles(n_nodes,
    ...
    if backend == 'IBMQX':
        ...
        var_form = IBMQXVarForm(
            problem_description, depth=backend_params['depth'])
...
# this is the new version, to be used in multiscale paper for ising objective. Old run_angles is preserved above
def run_and_get_best(x,
    ...
    if backend == 'IBMQX':
        if var_form_obj is not None:
            ...
            var_form = IBMQXVarForm(
                problem_description, depth=backend_params['depth'])
        resstrs = var_form.run(
            x, backend_name=backend_params['backend_device'], samples=5000)
    else:
        raise ValueError("Unsupported backend: {}".format(backend))
    energies = [
        (problem_description['objective_function'](problem_description, x), x) for x in resstrs
    ]
    if objective == 'min':
        return min(energies, key=itemgetter(0))
    elif objective == 'max':
        return max(energies, key=itemgetter(0))
    else:
