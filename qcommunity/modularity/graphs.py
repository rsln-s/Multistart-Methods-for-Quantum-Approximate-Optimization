...
def compute_modularity_dict(args, x):
    """
    Convenience function for refactored obj.py
    """
    try:
        return compute_modularity(args['n_nodes'], args['B'], x, C=args['C'])
    except KeyError:
        return compute_modularity(args['n_nodes'], args['B'], x)
...
def compute_cut_dict(args,x):
    return compute_cut(args['L'], x)

def compute_cut(L, bitstring):
    """
    L is graph Laplacian matrix, numpy/scipy matrix (preferably dense) 
    """
    if -1 in bitstring:
        # assuming bitstring is of zeros and ones
        bitstring = [
            0 if x == -1 else 1 if x == 1 else 'Error' for x in bitstring
        ]
    if 'Error' in bitstring:
        raise ValueError(
            "Incorrect bistring encountered. Only accepts bitstrings containing 0s and 1s or -1s and 1s of the size equal to number of nodes in G"
        )
    if not isinstance(bitstring, np.ndarray):
        bitstring = np.asarray(bitstring)
    if sp.sparse.issparse(L):
        L = L.todense()
    if not isinstance(L, np.ndarray) and L is not None:
        L = np.asarray(L)
    return float((bitstring.dot(L)).dot(bitstring.T))
...
def get_barbell_graph(left, right, **kwargs):
    ...
    size = left
    logging.info('Generating {}x{} barbell graph'.format(size, size))
...
def get_connected_caveman_graph(left, right, **kwargs):
    ...
    if left != right:
        ...
        ncliques = 2
        clique_size = left
    logging.info('Generating connected graph with {} cliques of size {} each'.format(
...
def get_erdos_renyi_graph(left, right, p=0.5, seed=42, **kwargs):
    right = int(right)
    # kwargs are ignored
    if left != right:
        warnings.warn("Ignoring right parameter")
    logging.info("Generating Erdos-Renyi graph on {} vertices with p={} and seed={}"
          .format(left, p, seed))
...
def remove_random_edge(G, edge_num = None):
    if edge_num == -1:
        logging.info("Choosing random edge...")
        edge = random.randint(0, G.number_of_edges()-1) # randint is inclusive: a <= N <= b
    else:
        edge = edge_num % (G.number_of_edges()-1)
        logging.info("Using passed in edge number {}".format(edge_num))
    edges = [(u,v) for u,v in G.edges()]
    logging.info("Removing edge ({},{}) out of {}".format(edges[edge][0], edges[edge][1], G.number_of_edges())) 
    G.remove_edge(edges[edge][0], edges[edge][1])
    logging.info("G now has {} edges".format(G.number_of_edges()))

if __name__ == "__main__":
    #G, solution_bitstring = get_random_partition_graph(
    #    8, 9, p_in=0.9, p_out=0.05, seed=42)
    G, solution_bitstring = get_connected_caveman_graph(2,5)
    ...
    #print(compute_modularity_c(G, solution_bitstring) / (4.0 * G.number_of_edges()))
    #cut = compute_cut(L, solution_bitstring)
    #print(cut)
    #colors = [
    #    'r' if x == -1 else 'b' if x == 1 else 'Error'
    #    for x in solution_bitstring
    #]
    #label = "Random Partition Graph Good"
    #nx.draw(G, node_color=colors, label=label, pos=pos)
    #import os.path
